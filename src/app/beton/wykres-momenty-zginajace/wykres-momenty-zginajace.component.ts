...
@Component({
  selector: 'app-wykres-momenty-zginajace',
  templateUrl: './wykres-momenty-zginajace.component.html',
  styleUrls: ['./wykres-momenty-zginajace.component.css']
})
export class WykresMomentyZginajaceComponent implements OnInit, OnChanges {
  ...
  yFn() {
    let d3 = this.d3;
    return d3.scaleLinear().domain([this.min(this.allData()), this.max(this.allData())]).range([this.lineYMarg, this.lineY * 2 - 2 * this.lineYMarg]);
  }
  xFn(daneGorneSet) {
    let d3 = this.d3;
